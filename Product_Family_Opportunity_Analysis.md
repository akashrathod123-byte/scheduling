...
- **Stud Plates Floor Plate Fasteners Insulation Screws and Concrete Spacers** — about **~120** in the world. about 7 hardware types x 5 sizes x 3 materials = ~105. *Check:* Hilti.

## Bucket 3 — Niche, Real Size+Material Depth (about 100–500)

**1,367 opportunities (11%).** 10 examples:
...
**Electrical & Electronic** — 220 opportunities
...
- *Power Distribution Switching* › **Medium-Voltage Protection Devices** — ~150. reclosers + sectionalizers + cutouts + fault limiters + grounding switches, several ratings each = ~100-200. *Check:* S&C Electric.
...
- *Drilling Accessories* › **Mag-Drill Adapters Extensions and Feeders** — ~120. vacuum plates + pipe adapters + extensions + transpointers + cable feeders + drill motors, many sizes = ~100+. *Check:* Hougen.
- *Machine Blades* › **Slitter Coater Demolition Machine and Perforator Blades** — ~150. about 6 blade types x 5 sizes x 5 machine models = ~150. *Check:* verify.
...
**Test/Measurement & Inspection** — 125 opportunities
...
- *Lab analyzers* › **Electrophoresis machines and well plates** — 100-300. a few machine types plus many well plate formats = ~200. *Check:* Bio-Rad.
- *Vacuum Process Instruments* › **Thin-Film Deposition Sensors and Controllers** — ~120. sensors + monitors + controllers + evap controllers + plasma arc, several each = ~100+. *Check:* INFICON.
- *Vacuum Process Instruments* › **Vacuum Gauges Transmitters and Controllers** — ~150. transmitters + controllers + diff switches, many ranges each = ~100-200. *Check:* Pfeiffer Vacuum.
...
- *Air Filtration* › **Air Filter Housings, Canisters and Cassettes** — ~150. filter housings, canisters, cassettes, conversion clips: several sizes x stages x media = ~150. *Check:* Camfil.
...
- *Bearings* › **Cutlass Bearings** — ~150. about 15 shaft sizes x 5 housing sizes x 2 materials = ~150. *Check:* Duramax Marine.
- *Linear Motion* › **Positioning Tables and Stages** — 100-300. precision tables, pivot, alignment stages/modules, ultra-high-load, telescoping rail, shaft+bearing, alignment: many travel ranges x axes x loads = a few hundred. *Check:* IKO.
...
**Plumbing/Pipe & Fluid Handling** — 76 opportunities
...
- *Quick-Connect Couplings* › **Specialty Multi-Connect and Auto-Dock Couplings** — ~150. auto dock + multi-connector + NFC + torque + auto-eject + supplied-air + paper mill + press/welder, several each = ~100-200. *Check:* CEJN.
...
**Safety/PPE & Facility** — 58 opportunities
...
- *Lawn Equipment* › **Lawn Sweepers Edgers Rollers Sod Cutters Tillers Dethatchers Mowers Wheel Hoes** — ~150. about 8 machine types x 5 sizes x 3 power options = ~150. *Check:* DR Power.
...
**Material Handling & Storage** — 57 opportunities
...
**Automation/Robotics & Controls** — 50 opportunities
- *SCADA & Field Controllers* › **Building and Energy Management Controllers** — ~120. building automation + energy mgmt + power meters + traffic signal, several models each = ~100+. *Check:* Honeywell.
- *SCADA & Field Controllers* › **Remote Terminal Units and Data Modules** — ~100. RTUs + DAQ modules + sensor uploaders + termination boxes, several channel counts each = ~100. *Check:* Schneider Electric.
- *Vehicle Electronics* › **Vehicle Controllers Cameras Sensors I/O Modules and Joysticks** — ~120. about 6 device types x 5 protocols x 4 configurations = ~120. *Check:* Parker.
- *Sensors - Specialty* › **Occupancy Bed Sleep Tire and Width Sensors** — ~100. occupancy + bed + sleep + tire + laser width, several each = ~100. *Check:* Garmin.
- *Sensors - Environmental* › **Gas CO2 Dewpoint and Wind Sensors** — ~120. gas + CO2 + dewpoint + wind sensors, many gas/range types each = ~100+. *Check:* Sensirion.
...
- *Sensors* › **Photoelectric fiber optic sensors** — 100-300. many sensing ranges x fiber types x housings = ~200. *Check:* Omron.
...
- *Decorative & Furniture Fasteners* › **Button Snap Fasteners and Eyelet Tools** — ~120. ball-head + fabric buttons + snap kits + eyelet tools, several each = ~100+. *Check:* YKK.
...
- *Hardware* › **Hinge accessories and cabinet doors** — 100-300. many hinge block sizes plus door sizes = ~200. *Check:* verify.
